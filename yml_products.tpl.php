...
$manufacter_field = variable_get('yml_export_manufacter_field', 'field_manufacter');
...
<vendors>
	...
	<vendor id="<?php echo $term -> tid ?>"<?php if($term -> parent): ?> parentId="<?php echo $term -> parent ?>"<?php endif; ?>><?php echo ys($term -> name)?></vendor>
	<?php endforeach; ?>
</vendors>
...
<offers>
  ...
  $brand = field_get_items('node', $node, $manufacter_field);
  if (isset($brand[0])) {
	$brand = $brand[0];
  }
...
?>   
	...
	<vendorCode><?php echo $brand['tid']; ?></vendorCode>
	<?php if (!empty($image)): ?>
